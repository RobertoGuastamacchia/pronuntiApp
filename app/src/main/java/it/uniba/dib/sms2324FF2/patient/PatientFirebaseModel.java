...
public class PatientFirebaseModel {
    ...
    public static void checkChildToken(UpdateTokenListener listener){
        FirebaseMessaging.getInstance().getToken()
                .addOnSuccessListener(new OnSuccessListener<String>() {
                    @Override
                    public void onSuccess(String token) {
                        getChildToken(Patient.getInstance().getId(), new PatientFirebaseModel.TokenListener() {
                            @Override
    ...
    public static void checkParentToken(UpdateTokenListener listener){
        FirebaseMessaging.getInstance().getToken()
                .addOnSuccessListener(new OnSuccessListener<String>() {
                    @Override
                    public void onSuccess(String token) {
                        getParentToken(Patient.getInstance().getId(), new PatientFirebaseModel.TokenListener() {
                            @Override
                        ...
                        if (task.isSuccessful()) {
                            ...
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                ...
                                db.collection("users").document(documentId).delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if(task.isSuccessful()){
                                            TherapistFirebaseAppointmentsModel.deleteAllAppointments(patientId, new TherapistFirebaseAppointmentsModel.OnDeleteListener() {
                                                @Override
