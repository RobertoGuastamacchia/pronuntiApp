...
public class MinimalPairsRecognitionFragment extends Fragment {
    ...
    private Uri imageUri, firstImgUri, secondImgUri;
    private ImageView imageChosen;

    SharedPreferences sharedPreferences;
    SharedPreferences.Editor editor;
    String patientId;
    private boolean isTheLastTime;
    int coin;
    Timestamp date;

    private onMinimalPairsRecognition mCallback;

    public interface onMinimalPairsRecognition {
        void showBottomBar();
        void hideBottomBar();
        void refreshPatientsData();
    }

    public MinimalPairsRecognitionFragment(boolean isPortrait, String id, int coin, Timestamp date){
    ...
    public MinimalPairsRecognitionFragment(boolean isPortrait, String id, boolean isTheLastTime, Uri firstImgUri, int coin, Timestamp date){
        this.isPortrait = isPortrait;
        this.patientId = id;
        this.isTheLastTime = isTheLastTime;
    ...
    public void onAttach(@NonNull Context context) {
        ...
        if (context instanceof onMinimalPairsRecognition) {
            mCallback = (onMinimalPairsRecognition) context;
        } else {
            throw new RuntimeException(context + " must implement onMinimalPairsRecognitionListener");
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view;
        if(isPortrait){
            view = inflater.inflate(R.layout.fragment_create_minimal_pairs_recognition, container, false);
        }else{
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if(isPortrait) {
            ...
            TextView upload_image_description = view.findViewById(R.id.upload_image_description);

            if(!isTheLastTime)
                upload_image_description.setText(getResources().getString(R.string.uploadFirstImage));
            else
                upload_image_description.setText(getResources().getString(R.string.uploadSecondImage));

            //gestione tasto prosegui
            finishButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    //invio la/le risposta/e al fragment giusto
                    //chiamo la pagina post-esercizio di caricamento

                    //vedo se devo chiamare il secondo fragment per registrazione dell'audio
                    if(!isTheLastTime)
                        getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.frameLayout, new MinimalPairsRecognitionFragment(true, patientId, true, firstImgUri, coin, date)).commit();
                    else
                        getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.frameLayout, new AddAudioToExerciseFragment(true, patientId, firstImgUri, secondImgUri, coin, date)).commit();
                }
            });
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
            getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.frameLayout, new MinimalPairsRecognitionFragment(false, patientId, coin, date)).commit();
        } else if (newConfig.orientation == Configuration.ORIENTATION_PORTRAIT) {
            getActivity().getSupportFragmentManager().beginTransaction().replace(R.id.frameLayout, new MinimalPairsRecognitionFragment(true, patientId, coin, date)).commit();
        }
